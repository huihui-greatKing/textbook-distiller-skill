...
- `scripts/enhance_practical_distillation.py`：对第一版结果做复习导向二次蒸馏，补充知识点解释、题型方法、作业写法和可直接使用的章节 Skill 内容。
- `scripts/upgrade_learning_distillation.py`：把章节 Skill 升级为“学习理解 + 复习整理 + 题型训练 + 作业书写辅助”的学习型 Skill。
...
- 实用产出优先：全量蒸馏的最终成果不能只停留在模板、清单或占位说明，必须生成能帮助复习和作业的内容，包括知识点解释、章节主线、常见题型、解题步骤、作业表达模板、易错点和自检题。
- 学习型蒸馏：教材蒸馏不是概要总结，而是把教材内容压缩成学生能学会、能复习、能练题、能完成作业的 Skill。每章必须讲清概念为什么引入、解决什么问题、和相近概念的区别、题目中如何考、作业中如何写。
...
如果第一版结果偏向结构化模板、占位清单或“后续再补充”，必须继续执行复习导向二次蒸馏：

```powershell
py skills/textbook-distiller/scripts/enhance_practical_distillation.py
```

二次蒸馏后至少应得到：
- `knowledge-base/normalized/study-guide.md`：整本书复习主线；
- `knowledge-base/normalized/concepts/chapter-xx-concepts.md`：每章知识点解释；
- `knowledge-base/normalized/problem-types/chapter-xx-problem-types.md`：每章题型和作业方法；
- `knowledge-base/normalized/chapter-summaries/chapter-xx-summary.md`：复习式章节总结；
- `skills/chapter-xx-english-name/SKILL.md`：可直接用于复习、解题和作业辅助的章节 Skill。

如果这些文件仍然只是空模板，应判定本轮蒸馏未达标，继续回到 OCR 证据、章节文本和课程知识框架重新修正。

如果二次蒸馏后仍然像章节概要，必须继续执行学习型深度蒸馏：

```powershell
py skills/textbook-distiller/scripts/upgrade_learning_distillation.py
```

学习型深度蒸馏的验收标准是：每章 Skill 必须包含学习目标、知识地图、核心概念精讲、公式规则方法、图表整理、典型题型、教材式解题流程、作业式答案模板、复习提纲、自测题与参考答案、常见错误和待人工复核项。
...
在阶段确认模式下，只有用户确认后执行；在全流程授权模式下，完成章节 OCR 和章节总结后可连续执行。每章生成 `skills/chapter-xx-english-name/SKILL.md`，并按学习型模板写入目标、学习目标、知识地图、核心概念精讲、公式规则方法、图表整理、典型题型、教材式表达、作业辅助模板、复习提纲、自测题与参考答案、常见错误、测试提示词和待复核项。

完整模板见 `references/templates.md`。

章节 Skill 如果只是本章简介、核心知识点列表和简单题型说明，应判定为不合格，必须重新生成。
